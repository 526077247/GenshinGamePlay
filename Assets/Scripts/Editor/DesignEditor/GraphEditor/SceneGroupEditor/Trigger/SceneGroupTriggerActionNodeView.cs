...
namespace TaoTie
{
    ...
    {
        public class SceneGroupTriggerActionNodeViewDrawBase : SceneGroupGraphDrawBase
        {
            private List<Type> subTypes;
            private string[] subNames;
            public SceneGroupTriggerActionNodeViewDrawBase(SceneGroupGraphWindow graphWindow): base(graphWindow)
            {
            ...
            protected override List<Type> GetSubClassList(FieldInfo field, object obj, Type type, out string[] names)
            {
                if (type == TypeInfo<ConfigSceneGroupAction>.Type)
                {
                    if (subTypes == null)
                    {
                        ...
                        {
                            if (subTypes[i].GetCustomAttribute(typeof(LabelTextAttribute)) is LabelTextAttribute
                                labelTextAttribute)
                            {
            ...
            {
                var graph = this.graphWindow.m_Graph;
                if (SceneGroupGraphDrawBase.RefreshValueDropDown(graph, field, obj, valuesGetter, valueDropdown))
                {
                    return;
                }
        ...
        {
            return new SceneGroupTriggerActionNodeViewDrawBase(graphWindow as SceneGroupGraphWindow);
        }
    }
}