	...
	public class AIDecisionInterface
	{
		...
		static AIDecisionInterface()
		{
			...
			for (int i = 0; i < methodInfos.Length; i++)
			{
				...
				var func = (Func<AIKnowledge, bool>) Delegate.CreateDelegate(TypeInfo<Func<AIKnowledge, bool>>.Type,
					null, methodInfos[i]);
		...
		/// <summary> 战斗追击移动条件。对应 AITactic.CombatFixedMove / MoveDecision.CombatFixedMove。
		/// 有目标且存在可到达目标最后感知位置(LKP)的寻路路径时，寻路向该位置移动。 </summary>
		public static bool IsCombatFixedMoveValid(AIKnowledge knowledge)
		{
			if (!knowledge.MoveKnowledge.CanMove) return false;
			if (knowledge.TargetKnowledge.TargetEntity == null) return false;
			if (knowledge.TargetKnowledge.TargetLKP == null) return false;
			return knowledge.ThreatLevel >= ThreatLevel.Alert
		...
		{
			if (!knowledge.MoveKnowledge.CanMove) return false;
			if (!knowledge.TargetKnowledge.HasLineOfSight) return false;
