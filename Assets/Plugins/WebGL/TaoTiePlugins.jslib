mergeInto(LibraryManager.library, {
    ...
    OpenUploader: function(){
        ...
        if(!uploader){
            ...
            '                var uploader1 = document.getElementById("unity-uploader");' +
            '                if(uploader1) uploader1.setAttribute("imgData" , e.target.result);' +
        ...
        uploader.removeAttribute("imgData");
        uploader.click();
    },
    GetImgData: function(){
        var uploader = document.getElementById("unity-uploader");
        if(uploader && uploader.hasAttribute("imgData")){
            var result = uploader.getAttribute("imgData");
            var bufferSize = lengthBytesUTF8(result) + 1;
            var buffer = _malloc(bufferSize);
            stringToUTF8(result, buffer, bufferSize);
            return buffer;
